...
# install.packages(c("sf", "knitr", "kableExtra", "tidycensus", "dplyr"), dependencies = TRUE)
...
```{r}
census_key <- readLines(here::here("Private", "census_api_key.txt"))
```
...
Great, now we have some info we can map, in order to plan our supermarkets.  Let's do a quick analysis of which variable might be most useful (statistically speaking):
...
```{r}
latino_palette <- colorBin("Greens", domain = philly_latino_wide$pct_latino, bins = 5, na.color = "#808080")

leaflet() %>%
addPolygons(
    data = philly_latino_wide,
